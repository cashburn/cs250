 ...
 *
 ***/
    char * start = (char *) ((int)p - ((int)p % 16));
    int end = len + (16 - ((int)p % 16));

    printf("0x%p: ", (unsigned long) start);
    int i;
    for (i = 0; start + i < p; i++) {
        printf(".. ");
    }
    for (i = i; i < len + 16; i++) {
        ...
        if (((i + 1) % 16) == 0) {
            ...
            for (int j = (i - 15); j <= i; j++) {
                if (((start[j]&0xFF) <= 127) && ((start[j]&0xFF) >= 32)
                    && ((start+j) >= p) && (start+j < p + len))
                    printf("%c", start[j]&0xFF);
                else
                    printf(".");
            }
            printf("\n");
            if ((i + 1) < end)
                printf("0x%p: ", (unsigned long) (start + i + 1));
        }
